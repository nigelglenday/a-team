...
### Added
- **Window groups, nestable.** An agent's `group` says which window it belongs in, a different axis from `category`. Paths nest (`backoffice/sidekick`), and which node becomes a window is declared separately with `a-team config split-windows`, so the layout can change without re-tagging anything. `a-team layout` rebuilds the whole screen, `a-team groups` shows the tree, `a-team set <agent> --group <path>` assigns one.
