...
# Any agent can be fenced, not just the client-facing ones: an Associate to one
# customer's tenant, the Bookkeeper to the bank and ledger, the Concierge to
# mail and calendar. The account name is the key; what it may reach is declared
# once in org.json and generated by `agent-grants`.
#
...
# by `agent-grants`, so derive both here from the same name.
GRANT_MCP_DIR = "~/.config/grants/mcp"
GRANT_SETTINGS_DIR = "~/.config/grants/perms"
